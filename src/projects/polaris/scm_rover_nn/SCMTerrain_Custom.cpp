...
SCMTerrain_Custom::SCMTerrain_Custom(ChSystem* system, bool visualization_mesh, bool use_nn, int num_wheels) :SCMTerrain(system,visualization_mesh) {
    m_loader = chrono_types::make_shared<SCMLoader_Custom>(system, visualization_mesh, use_nn, num_wheels);
    system->Add(m_loader);    
}

// Initialize the terrain as a flat grid.
void SCMTerrain_Custom::EnterVehicle(std::shared_ptr<Curiosity> vehicle, int id_vehicle) {
...
// Constructor.
SCMLoader_Custom::SCMLoader_Custom(ChSystem* system, bool visualization_mesh, bool use_nn, int num_wheels) : m_sys(system), m_soil_fun(nullptr) {
...
void SCMLoader_Custom::EnterVehicle(std::shared_ptr<Curiosity> vehicle, int id_vehicle) {

    //m_vehicle = vehicle;

    // Pablo
    // for (int i = 0; i < m_num_wheels; i++) {
    //     m_wheels[i] = vehicle->GetWheel(i);
    // }

    //std::cout << CuriosityWheelID::C_LF << std::endl;

    m_wheels[0] = vehicle->GetWheel(CuriosityWheelID::C_LF);
    m_wheels[1] = vehicle->GetWheel(CuriosityWheelID::C_RF);
    m_wheels[2] = vehicle->GetWheel(CuriosityWheelID::C_LM);
    m_wheels[3] = vehicle->GetWheel(CuriosityWheelID::C_RM);
    m_wheels[4] = vehicle->GetWheel(CuriosityWheelID::C_LB);
    m_wheels[5] = vehicle->GetWheel(CuriosityWheelID::C_RB);

    // Set default size and offset of sampling box
    tire_radius = 0.25;//m_wheels[0]->radius;
    tire_width = 0.40;//m_wheels[0]->width;
...
void SCMLoader_Custom::Initialize(const std::string& heightmap_file,
    ...
    for (int ix = 0; ix < nvx; ix++) {
        ...
        for (int iy = 0; iy < nvy; iy++) {
            ...
            assert(ay < 1.0);
            assert(jy1 < ny_img);
            // Pablo: need to comment when using height map
            //assert(jy2 < ny_img);
            assert(jy1 <= jy2);
...
void SCMLoader_Custom::ComputeInternalForces() {
    ...
    for (int i = 0; i < m_num_wheels; i++) {
        ...
        w_rot[i] = m_wheels[i]->GetRot();
        //w_nrm[i] = w_state.rot.GetYaxis();
        w_linvel[i] = m_wheels[i]->GetLinVel();
        w_angvel[i] = m_wheels[i]->GetAngVel();

        w_contactable[i] = m_wheels[i]->GetBody()->GetCollisionModel()->GetContactable();
...
// #endif        
    ...
    // End standard raycasting computation
    ...
    // std::array<ChVector<float>, m_num_wheels> wheel_force;
    // for (int i=0; i<m_num_wheels; i++){
    ...
    for (auto& h : newhits) {
        ...
        if (nr.sigma > nr.sigma_yield) {
            // Bekker formula
            // TO DO get correct fornmula for OOB, right now, Kc=0 so not needed (or use ~5, which is the mean value)
            ...
            nr.sigma = Bekker_Kphi * pow(nr.sinkage, Bekker_n);
